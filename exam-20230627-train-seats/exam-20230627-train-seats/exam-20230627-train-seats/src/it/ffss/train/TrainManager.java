...
public class TrainManager {
	...
	Map<String, Car> trainCars;
	Map<String, Stop> trainStops;
	Map<String, Booking> trainBookings;
	
	
	Integer currentStopPosition = 1;
	Integer currentBookingID = 1;
//R1
	...
	public int addCar(String id, int rows, char lastSeat, String klass) throws TrainException {
		...
		Car car = new Car(id,rows,lastSeat,klass);
		trainCars.put(id, car);
	...
	public Collection<String> getCarsByClass(String klass) {
		return trainCars.values().stream().filter(c -> c.getKlass().equals(klass)).map(c -> c.getId()).collect(Collectors.toList());
	}
	...
	public int getNumSeats(String id) {
		Car c = trainCars.get(id);
		return c.getSeatCount();
	}
	...
	public int defineStops(String... stops) {
		if(trainStops==null) {
			trainStops = new TreeMap<>();
		}
		
		for (String stop : stops) {
			Stop s = new Stop(stop, currentStopPosition);
			trainStops.put(stop, s);
			currentStopPosition++;
		}
	...
	public Map<String, List<String>> findSeats(String begin, String end, String klass) {
		...
		if(trainBookings == null) {
			for (Car c : trainCars.values()) {
				List<String> remainingSeats = c.generateAllCarSeats();
				map.put(c.getId(),remainingSeats);
			}
		} else {
			for (Car c : trainCars.values()) {
				List<Booking> booked = trainBookings.values().stream().filter(b -> trainCars.get(b.getCar()).getKlass().equals(klass))
						.filter(b -> trainStops.get(b.getEnd()).getStopPosition() >= trainStops.get(begin).getStopPosition() && trainStops.get(b.getBegin()).getStopPosition() <=  trainStops.get(end).getStopPosition()).toList();
				List<String> bookedSeats = booked.stream().map(b -> b.getSeat()).distinct().collect(Collectors.toList());
				List<String> remainingSeats = new ArrayList<>();
				for(String seat : c.generateAllCarSeats()) {
	...
	public String bookSeat(String ssn, String name, String surname, 
		...
		Car c = trainCars.get(car);
		if (c == null) throw new TrainException("Car not valid " + c);
		
		if (!(trainStops.containsKey(begin) && trainStops.containsKey(end))) throw new TrainException("Stops not valid " + begin  + " "+ end);
		
		Map<String, List<String>> allSeats = this.findSeats(begin, end, c.getKlass());
		if (!allSeats.get(c.getId()).contains(seat)) throw new TrainException("Booked seat");
		
		Booking booking = new Booking(ssn,name,surname,begin,end,car,seat);
		booking.setBookingID(String.valueOf(currentBookingID));
		currentBookingID++;
		
		trainBookings.put(booking.getBookingID(), booking);
